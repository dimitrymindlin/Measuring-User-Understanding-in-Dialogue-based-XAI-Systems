...
class SimilarInstancesByExplanation(object):
    def __init__(self,
                 explainer,
                 data,
                 predict_fn,
        ...
        Sets value:
            sp_explanations: A list of explanation objects that are most similar
        ...
        # Now run the SP-LIME greedy algorithm with similarity objective
        ...
        for _ in range(num_exps_desired):
            ...
            for i in remaining_indices:
                similarity = self._compute_similarity(W[i], W[V])
    ...
    def _compute_similarity(self, vec, matrix):
        # Compute cosine similarity between 'vec' and each row in 'matrix'
        norm_vec = np.linalg.norm(vec)
        norm_matrix = np.linalg.norm(matrix, axis=1)
        similarity = np.dot(matrix, vec) / (norm_matrix * norm_vec)
